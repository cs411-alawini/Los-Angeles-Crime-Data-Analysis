...
router.get('/', async function(req, res) {
    ...
    try {
        ...
        let hourResult = month_hour_result.slice(12);

        // the hourResult is not sorted by time yet
        hourResult.sort((x, y) => {
            // each timeSlot is like '04:00:00-04:59:00', we just need the hour to sort
            // convert to integer so we can subtract
            x_hour = parseInt(x.timeSlot.split(':')[0]);
            y_hour = parseInt(y.timeSlot.split(':')[0]);
            // if > 0, x is larger, =0 equal, <0 smaller
            return x_hour - y_hour;
        });
